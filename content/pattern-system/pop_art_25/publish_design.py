...
def ensure_source_png(base_id:str,collection:dict):
    ...
    if src.is_file():
        ...
        if amin==0 and amax>0:
            if existing.size==(800,960):
                return src
            # The 24 branch masters were staged at 500x600. Scale them to the
            # canonical 800x960 canvas with nearest-neighbour so palette pixels
            # stay exact and transparency stays crisp.
            if existing.width*6==existing.height*5:
                normalized=existing.resize((800,960),Image.Resampling.NEAREST)
                normalized.save(src,"PNG",optimize=True)
                print(f"SOURCE_NORMALIZED {base_id} {existing.size} -> {normalized.size}")
                return src
    ...
    print(f"SOURCE_REBUILT {base_id} {src}")
    return src
