...
def cmd_next(args):
    doc = load()
    row = None if not doc.get("auto_continue", False) else next((x for x in doc["items"] if x.get("status") == "pending"), None)
    values = {
...
def cmd_has_work(args):
    ...
    print("yes" if doc.get("auto_continue", False) and pending else "no")
