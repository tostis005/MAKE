...
QUEUE = ROOT / "content" / "pattern-system" / "florals" / "publish_queue.json"
...
def cmd_next(args):
    doc = load()
    item = None if not doc.get("auto_continue", True) else next(
        (x for x in doc["items"] if x.get("status") == "pending"), None
    )
...
def main():
    ...
    p = sub.add_parser("has-work")
    p.set_defaults(func=cmd_has_work)
